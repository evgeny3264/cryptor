...
unsigned long int P = 0xb7e15163, Q = 0x9e3779b9;  /* magic constants             */
...
void RC5_ENCRYPT(unsigned long int *pt, unsigned long int *ct) /* 2 unsigned long int input pt/output ct    */
{
...
void RC5_DECRYPT(unsigned long int *ct, unsigned long int *pt) /* 2 unsigned long int input ct/output pt    */
{
...
int crypt(unsigned char* in, int sizein, unsigned char * key, unsigned char*& out)
{
	...
	if (sizein % 8 != 0)
	{
		...
		int newSize;
		newSize = sizein + (8 - (sizein % 8));
		in = new unsigned char[newSize];
		memcpy(in, temp, sizein);
		for (int i = sizein; i < newSize; i++)
		{
			in[i] = 0;
		}
		sizein = newSize;
	...
	unsigned long int ot[2], ct[2] = { 0, 0 };
	out = new unsigned char[sizein];
	RC5_SETUP(key);
	for (int i = 0; i < sizein; i += 8)
	{
		ot[0] = *(unsigned long int*)& in[i];
		...
		ot[1] ^= ct[1];
		RC5_ENCRYPT(ot, ct);
		*(unsigned long int*)&out[i] = ct[0];
		*(unsigned long int*)&out[i + 4] = ct[1];
	}
	if (resize){
		delete[] in;
	}
...
int decrypt(unsigned char* in, int sizein, unsigned char * key, unsigned char*& out){
	...
	for (int i = 0; i < sizein; i += 8)
	{
		...
		ctprv[0] = ct[0];
		ctprv[1] = ct[1];
		*(unsigned long int*)&out[i] = ot[0];
		*(unsigned long int*)&out[i + 4] = ot[1];
	}
	return sizein;
}
